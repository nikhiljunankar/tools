rem This script performs an rclone sync from the local SOURCE folder to a GCS bucket.
...
set "LOCAL_SOURCE_PATH=C:\Users\nikhi\tools\rclone\logs"
...
echo [%DATE% %TIME%] Starting rclone sync from SOURCE to GCS... >> "%RCLONE_LOG_FILE_PATH%"

rem --- Change Working Directory ---
cd "%LOCAL_SOURCE_PATH%"
if %ERRORLEVEL% NEQ 0 (
    echo [%DATE% %TIME%] ERROR: Failed to change directory to "%LOCAL_SOURCE_PATH%". Exiting. >> "%RCLONE_LOG_FILE_PATH%"
...
"%RCLONE_EXE_PATH%" sync "%LOCAL_SOURCE_PATH%" "%GCS_REMOTE_DEST%" --config "%RCLONE_CONFIG_PATH%" --log-file "%RCLONE_LOG_FILE_PATH%" --log-level DEBUG --fast-list
